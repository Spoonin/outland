...
# Authored graph (~17 nodes). Bulk = heavy/cheap/shallow; hi-tech = light/dear/deep.
GRAPH: List[Node] = [
    Node("water",       1, 100, 1,    2.0, ()),
    Node("oxygen",      1, 20,  1,    1.0, (("water", 0.3),), crit=1.0),
    Node("steel",       1, 200, 2,    0.3, (), crit=0.2),
    Node("metals",      2, 60,  8,    0.0, (), crit=0.2),
    Node("hydrogen",    2, 2,   5,    0.0, (("water", 1.0),), crit=0.3),
    Node("food",        2, 50,  5,    1.5, (("water", 0.5), ("fertilizer", 0.2)), crit=1.0),
    Node("fuel",        3, 30,  30,   0.2, (("hydrogen", 0.5), ("catalyst", 0.03)), crit=0.3),
    Node("ammonia",     3, 8,   15,   0.0, (("hydrogen", 0.3), ("catalyst", 0.02)), crit=0.4),
    Node("fertilizer",  3, 10,  20,   0.0, (("ammonia", 0.5), ("catalyst", 0.05)), crit=0.6),
    Node("plastics",    4, 15,  40,   0.4, (("fuel", 0.2), ("catalyst", 0.05)), crit=0.3),
    Node("alloy",       5, 12,  150,  0.0, (("steel", 0.4), ("metals", 0.2), ("precision", 0.01)), crit=0.4),
    Node("machinery",   4, 40,  80,   0.3, (("steel", 0.5), ("electronics", 0.1), ("alloy", 0.1)), crit=0.5),
    Node("maternity",   4, 30,  100,  0.0, (("electronics", 0.05), ("machinery", 0.1))),  # infra → births
    # black nodes: deep, light (cheap shipping) but DEAR by intrinsic cost, critical, low per-cap use
    Node("catalyst",    6, 0.5, 8.0e7,  0.0,  (), black=True, crit=0.8),
    Node("precision",   6, 1,   1.2e8,  0.0,  (), black=True, crit=0.5),
...
class Params:
    M: float = 1.0e12
    c: float = 2.0e5                 # money per kg landed
    mes0: float = 300.0
    k: float = 2.0
    capital_factor: float = 1.0e9
...
def import_floor(s: State, nd: Dict[str, float], price_mult: float = 1.0) -> float:
    """F (money): imported portion of every node, priced (earth_cost + mass*c)*infl/spike."""
    p = s.p
    f = 0.0
    for n in GRAPH:
        price = (n.earth_cost + n.mass * p.c) * price_mult
        imported_units = nd[n.name] * (tail_frac(p, s.age[n.name]) if s.localized[n.name] else 1.0)
        f += imported_units * price
    if s.fusion == "online":
...
def step(s: State) -> dict:
    ...
    nd = needs(s)
    f_pre = import_floor(s, nd, price_mult)
    ...
    nd = needs(s)
    f = import_floor(s, nd, price_mult)
    free = m_eff - f
    ...
    if free >= 0:
        ...
        if spare > p.colonist_cost:
            ...
            s.pop += new
        # births only if maternity infra localized
        if s.localized["maternity"]:
